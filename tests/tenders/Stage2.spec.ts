...
test('Проверить форму этапа 2', async ({page}) => {
    ...
    // добавить проверку на удаление, но только после создания
    ...
    await page.waitForTimeout(500);
    await page.locator('#RebPrice708').click();
    await page.locator('#RebPrice708').fill('100.12₽');
    ...
    await expect(page.getByLabel('Формы 2 этапа')).toMatchAriaSnapshot(`
      ...
      - button "Удалить"
      `);
    await expect(page.getByLabel('Переторжка 1')).toMatchAriaSnapshot(`
      ...
      `);
    await expect(page.locator('#Stage2FormPrice19')).toHaveValue('11,703.98₽');
    await expect(page.locator('#RebPrice708')).toHaveValue('100.12₽');
