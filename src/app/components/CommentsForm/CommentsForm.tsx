...
const CommentsForm: React.FC<CommentsFormProps> = observer(({ tender }) => {
    ...
    return (
        <div className={`card ${styles.form} ${collapsed.isTrue ? styles.expanded : ''}`}>
            <h3>Комментарии <button className={styles.toggler} onClick={collapsed.toggle}><FontAwesomeIcon icon={faCaretUp} className={`${styles.icon} ${!collapsed.isTrue ? styles.rotated : ''}`} /></button></h3>
            {comments}
        </div>
    )
});
