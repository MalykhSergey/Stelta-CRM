...
export async function authAction<T>(handler: (user: User) => Promise<T>) {
    ...
    if (auth_token) {
        try {
            ...
            if (found_user != null && found_user.password == user_in_token.password && found_user.role != Role.Viewer) {
            ...
            if (e instanceof jwt.TokenExpiredError) {
                ...
                if (found_user != null && found_user.password == decoded_token.password && found_user.role != Role.Viewer) {
                    const new_token = jwt.sign(
                        { id: decoded_token.id, name: decoded_token.name, password: decoded_token.password, salt: decoded_token.salt, role: decoded_token.role },
                        process.env.JWT_SECRET!,
                        { expiresIn: expirationTime }
                    )
